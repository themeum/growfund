...
class MaxRule extends BaseRule
{
    ...
    public function get_error_message()
    {
        if ($this->is_string_value()) {
            ...
            return sprintf(__('The %1$s field must be less than or equal to %2$s characters.', 'growfund'), $this->key, $this->rule_value);
        } elseif ($this->is_array_value()) {
            /* translators: 1: field name, 2: rule value */
            return sprintf(__('The %1$s field must be less than or equal to %2$s items.', 'growfund'), $this->key, $this->rule_value);
        }

        /* translators: 1: field name, 2: rule value */
        return sprintf(__('The %1$s field must be less than or equal %2$s.', 'growfund'), $this->key, $this->rule_value);
    }
