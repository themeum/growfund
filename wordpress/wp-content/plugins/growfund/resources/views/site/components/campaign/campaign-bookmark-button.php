...
<?php if (!$campaign_bookmark_button->campaign->is_ended) : ?>
    <button 
        ...
        class="growfund-campaign-bookmark-button <?php echo $campaign_bookmark_button->classname ? esc_attr($campaign_bookmark_button->classname) : ''; ?> <?php echo $campaign_bookmark_button->campaign->is_bookmarked ? esc_attr('growfund-campaign-is-bookmarked') : ''; ?>"
        aria-label="Bookmark campaign"
        data-campaign-id="<?php echo esc_attr($campaign_bookmark_button->campaign->id); ?>"
        data-campaign-is-bookmarked="<?php echo $campaign_bookmark_button->campaign->is_bookmarked ? 'true' : 'false'; ?>"
    ...
    >
        ...
        <?php if ($campaign_bookmark_button->has_label) : ?>
            <span class="growfund-campaign-bookmark-button-label" data-bookmarked-label="<?php echo esc_attr($growfund_bookmarked_label); ?>" data-bookmark-label="<?php echo esc_attr($growfund_bookmark_label); ?>">
                <?php echo $campaign_bookmark_button->campaign->is_bookmarked ? esc_html($growfund_bookmarked_label) : esc_html($growfund_bookmark_label); ?>
            </span>
        <?php endif; ?>
    </button>
<?php endif; ?>