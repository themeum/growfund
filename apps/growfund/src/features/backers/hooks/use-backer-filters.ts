...
import { type BackerFilterForm } from '@/features/backers/schemas/backer-filter';
...
interface UseBackerFiltersOptions {
  form: UseFormReturn<BackerFilterForm>;
  syncQueryParams: (values: Partial<Values<UseQueryStatesKeysMap>>) => void;
  params: Values<UseQueryStatesKeysMap>;
}

export function useBackerFilters({ form, syncQueryParams, params }: UseBackerFiltersOptions) {
  ...
  const { handleClearFilter, handleClearAllFilters } = useFilterClearing({
    ...
    transformParamsToFormData: (params: Values<UseQueryStatesKeysMap>): BackerFilterForm => {
      return {
        search: params.search as string | undefined,
        status: params.status as 'all' | 'trashed' | undefined,
