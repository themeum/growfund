...
import { type PledgeFilter } from '@/features/pledges/schemas/pledges-filter';
import { useFilterClearing } from '@/hooks/use-filter-clearing';

interface UsePledgeFiltersOptions {
  form: UseFormReturn<PledgeFilter>;
  syncQueryParams: (values: Partial<Values<UseQueryStatesKeysMap>>) => void;
  params: Values<UseQueryStatesKeysMap>;
}

export function usePledgeFilters({ form, syncQueryParams, params }: UsePledgeFiltersOptions) {
  ...
  const { handleClearFilter, handleClearAllFilters } = useFilterClearing({
    ...
    transformParamsToFormData: (params: Values<UseQueryStatesKeysMap>): PledgeFilter => {
      return {
