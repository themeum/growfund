...
import {
  ...
  type PdfReceiptTemplateForm as PdfReceiptTemplateType,
} from '@/features/settings/schemas/pdf-receipt';
import { useGetOptionQuery } from '@/services/app-config';

const PdfDonationReceiptTemplate = () => {
  const formRef = useRef<HTMLFormElement | null>(null);

  const pdfReceiptTemplateQuery = useGetOptionQuery(OptionKeys.PDF_DONATION_RECEIPT_TEMPLATE);

  const form = useForm<PdfReceiptTemplateType>({
    resolver: zodResolver(PdfReceiptTemplateSchema),
  });

  const { registerForm } = useTemplateLayoutContext<PdfReceiptTemplateType>();

  useEffect(() => {
    const cleanup = registerForm(OptionKeys.PDF_DONATION_RECEIPT_TEMPLATE, form);
    return () => {
      cleanup();
    };
  ...
  return (
    <TemplateForm form={form} ref={formRef}>
      <PdfReceiptTemplateForm pdfReceiptTemplate={pdfReceiptTemplate} />
    </TemplateForm>
  );
};

export default PdfDonationReceiptTemplate;
